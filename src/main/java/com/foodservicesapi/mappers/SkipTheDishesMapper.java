...
public interface SkipTheDishesMapper {
  ...
  @Mappings({
          ...
          @Mapping(target = "extensions.persistedQuery.sha256Hash", constant = "e4650bd94c9d17af7fbc24dfc1475a6fb6f496a49d9e967dc40ef4e40cf747bc"),
          @Mapping(target = "variables.isDelivery", constant = "true"),
          @Mapping(target = "variables.language", constant = "en"),
          @Mapping(source = "city", target = "variables.city"),
          @Mapping(source = "province", target = "variables.province"),
          @Mapping(source = "latitude", target = "variables.latitude"),
          @Mapping(source = "longitude", target = "variables.longitude"),
  })
  ...
  RestaurantDetailsRequestSkipTheDishes toRestaurantDetailsRequest(Address address, String restaurantId);
  ...
  List<RestaurantOverview> toRestaurantOverviewDomainList(List<RestaurantsListResponseOpenRestaurantsSkipTheDishes> restaurants);
  @Mappings({
          @Mapping(source = "cleanUrl", target = "id"),
          @Mapping(source = "name", target = "name"),
          @Mapping(source = "skipScore", target = "rating", qualifiedByName = "skipScoreToDomainRatingScale"),
          @Mapping(source = "imageUrls.menuSmallUrl", target = "imageUrl"),
          @Mapping(target = "serviceProvider", constant = "SKIPTHEDISHES")
  })
  RestaurantOverview toRestaurantOverviewDomain(RestaurantsListResponseOpenRestaurantsSkipTheDishes restaurant);
  ...
  @Mappings({
          ...
          @Mapping(source = "cleanUrl", target = "redirectUrl", qualifiedByName = "skipTheDishesRedirectUrl"),
          @Mapping(source = "skipScore", target = "rating", qualifiedByName = "skipScoreToDomainRatingScale"),
  ...
  Restaurant toRestaurantDomain(RestaurantDetailsResponseWrapperSkipTheDishes skipTheDishesRestaurant);
  ...
  Fee toFeeDomain(RestaurantDetailsResponseFeeSkipTheDishes feeSkipTheDishes, double maximumCartSubtotal);

  @Mappings({
          @Mapping(source = "menuItems", target = "items")
  })
  MenuCategory toMenuCategoryDomain(RestaurantDetailsResponseMenuGroupSkipTheDishes menuGroupSkipTheDishes);
  ...
  default List<Fee> skipTheDishesFees(List<RestaurantDetailsResponseFeeSkipTheDishes> feesSkipTheDishes) {
    List<Fee> feeArrayList = new ArrayList<>();

    feesSkipTheDishes.sort((o1, o2) -> Double.compare(o2.getOrderMinimumCents(), o1.getOrderMinimumCents()));
    for (int i = 0; i < feesSkipTheDishes.size(); i++) {
      feeArrayList.add(toFeeDomain(feesSkipTheDishes.get(i), i > 0 ? feesSkipTheDishes.get(i-1).getOrderMinimumCents() : Integer.MAX_VALUE));
    }
  ...
  static double skipScoreToDomainRatingScale(double skipScore) {
    return Math.round((skipScore * (10 / skipScoreOutOf) * 100)) / 100;
  }
